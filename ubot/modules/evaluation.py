...
async def userprofilegetter(event):
    if event.args:
        ...
        if reply_id:
            ...
            await event.reply("The user may have privacy settings enabled!")
            return
    else:
...
async def flipsticker(event):
    ...
    if not sticker:
        await event.reply("Reply to a sticker to flip it")
        return
...
async def compressor(event):
    ...
    if not sticker:
        await event.reply("Reply to a sticker to compress it!")
        return
